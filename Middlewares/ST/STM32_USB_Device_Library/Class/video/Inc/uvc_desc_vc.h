...
*/
	SIZEOF_M(struct usbd_uvc_cfg, uvc_vc_xu_vigil) +
...
.uvc_vc_xu_vigil = {
  .bLength =
    SIZEOF_M(struct usbd_uvc_cfg, uvc_vc_xu_vigil), // Descriptor size
  .bDescriptorType = 0x24,                     // Class specific interface desc type
  .bDescriptorSubType = 0x06,                  // Extension Unit Descriptor type
  .bUnitID = VC_CONTROL_XU_VIGIL,     // ID of this terminal
  .guidExtensionCode = {                       // 16 byte GUID
    'a','d','e','-',
    'v','i','g','i',
    'l','8','0','-',
