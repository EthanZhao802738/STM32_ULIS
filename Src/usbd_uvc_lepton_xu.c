...
{
	VIGIL_RESULT result = VIGIL_OK;

	if (entity_id == VC_CONTROL_XU_VIGIL)
	{
		if (offset < VIGIL_FUNC_T || offset >= VIGIL_FUNC_END)
			result = VIGIL_UNDEFINED_FUNCTION_ERROR;
		else
		{
			switch(offset)
			{
			case VIGIL_FUNC_T:
				//get (0, 0) temperature? or center
				result = VIGIL_OK;
				break;

			case VIGIL_FUNC_C:
				result = VIGIL_COMMAND_NOT_ALLOWED;
				break;
		...
		result = VIGIL_FUNCTION_NOT_SUPPORTED;
